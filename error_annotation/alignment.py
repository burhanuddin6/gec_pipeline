...
class Alignment:
    ...
    def align(self, lev):
        # Sentence lengths
        o_len = len(self.orig.words)
        c_len = len(self.cor.words)
        # Lower case token IDs (for transpositions)
        o_low = [o for o in self.orig.words]
        c_low = [c for c in self.cor.words]
        ...
        orig = self.orig.words
        cor = self.cor.words
        # Loop through the cost_matrix
        for i in range(o_len):
            for j in range(c_len):
                # Matches
                if orig[i].text == cor[j].text:
                ...
                # Non-matches
                    ...
                    # Linguistic Damerau-Levenshtein
                        ...
                        sub_cost = cost_matrix[i][j] + \
                            self.get_sub_cost(orig[i], cor[j])
                        ...
                        while i-k >= 0 and j-k >= 0 and \
                            ...
                            if sorted(o_low[i-k:i+1], key=lambda x: x.text) == sorted(c_low[j-k:j+1], key=lambda x: x.text):
    ...
    def __str__(self):
        orig = " ".join(["Orig:"]+[tok.text for tok in self.orig.words])
        cor = " ".join(["Cor:"]+[tok.text for tok in self.cor.words])
